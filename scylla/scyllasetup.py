...
class ScyllaSetup:
    ...
    def arguments(self):
        ...
        args += ["--skip-wait-for-gossip-to-settle 0"]
        args += ["--enable-sstable-data-integrity-check false"]
        args += ["--enable-in-memory-data-store true"]
        args += ["--enable-cache false"]
        args += ["--enable-commitlog false"]
        args += ["--num-tokens 1"]
